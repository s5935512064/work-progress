...
const URL = process.env.NEXT_PUBLIC_API_URL + `api/job-running`;
const fetcher = (url: string) => axios.get(url).then((res) => res.data);

const RunningJob: FC<Props> = (): JSX.Element => {
  const { data: job_running, error: function_error } = useSWR(URL, fetcher);

  useEffect(() => {
    console.log(job_running);
  });
    ...
    <>
      <div className="w-full pt-2">
        <div className="w-full flex flex-col gap-4 ">
          {job_running.map((item: any, index: number) => (
            <div key={index}>
              <Disclosure>
                {({ open }) => (
                  <>
