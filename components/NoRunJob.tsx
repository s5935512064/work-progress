...
const URL = process.env.NEXT_PUBLIC_API_URL + `api/job-norun`;
const fetcher = (url: string) => axios.get(url).then((res) => res.data);

const NoRunJob: FC<Props> = (): JSX.Element => {
  const { data: job_norun, error: function_error } = useSWR(URL, fetcher);

  useEffect(() => {
    console.log(job_norun);
  });
    ...
    <>
      <div className="w-full pt-2">
        <div className="w-full flex flex-col gap-4 ">
          {job_norun.map((item: any, index: number) => (
            <div key={index}>
              <Disclosure>
                {({ open }) => (
                  <>
